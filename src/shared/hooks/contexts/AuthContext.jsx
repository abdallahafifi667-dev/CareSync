...
import { allUsers as initialAllUsers, addUser, usersByEmail } from "../../data/dummyData";
...
export const AuthProvider = ({ children }) => {
  ...
  // Login with Google (Firebase) - DISABLED as per user request
  const loginWithGoogle = async () => {
    throw new Error("Google login is currently disabled.");
  };

  // Backend API login with Mock bypass
  const login = async (email, password, role) => {
    setLoading(true);
    try {
      if (import.meta.env.VITE_USE_MOCK_AUTH === 'true') {
        const mockUser = usersByEmail[email];
        if (mockUser && mockUser.password === password) {
          const userWithRole = { ...mockUser, role: mockUser.role || role };
          setUser(userWithRole);
          localStorage.setItem("caresync_user", JSON.stringify(userWithRole));
  ...
  // Backend API register with Mock bypass
