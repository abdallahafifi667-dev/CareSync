...
export default function Sidebar() {
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <aside
        className={`fixed top-0 left-0 min-h-screen ${sidebarWidth} bg-gradient-to-b from-primary-50/90 via-white to-medical-50/90 shadow-xl border-r border-subtle flex flex-col z-50 transition-transform duration-200 ${open ? "translate-x-0" : "-translate-x-full"
          ...
          <ul className="space-y-1">
            {menuItems.map((item) => (
              <li key={item.name}>
                <Link
                  to={item.href}
                  className={`flex items-center px-4 py-2 text-base rounded-lg transition-colors group ${isActiveLink(item.href)
                ...
                >
                  <item.icon
                    className={`mr-3 h-6 w-6 ${isActiveLink(item.href)
